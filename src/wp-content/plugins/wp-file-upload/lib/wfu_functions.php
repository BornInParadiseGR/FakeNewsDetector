...
function wfu_create_random_string($len, $hex = false) {
	...
	else mt_srand((int)((double)substr(uniqid("", true), 15)));
	$is_first = true;
 ...
 * Sanitize a Hex.
 *
 * This function sanitizes a hex string.
 *
 * @since 4.24.0
 *
 * @param string $hex The hex string to sanitize.
 *
 * @return string The sanitized hex.
 */
function wfu_sanitize_hex($hex) {
	return preg_replace("/[^A-Fa-f0-9]/", "", $hex);
}

/**
 * Sanitize a Base64 String.
 *
 * This function sanitizes a base64 string.
 *
 * @since 4.24.0
 *
 ...
 */
function wfu_sanitize_base64($base64) {
	return preg_replace("/[^a-zA-Z0-9\/+=]/", "", $base64);
}

/**
 * Sanitize a Dotted Base64 String.
 *
 * This function sanitizes a base64 string which also contains dot (.) symbols
 * as separators of the string into parts.
 *
 * @since 4.24.0
 *
 * @param string $base64dt The dotted base64 string to sanitize.
 *
 * @return string The sanitized string.
 */
function wfu_sanitize_base64dotted($base64dt) {
	return preg_replace("/[^a-zA-Z0-9\/+=.]/", "", $base64dt);
}
...
function wfu_get_filtered_recs($filter) {
	...
	// construct upload form IDs filter
	if ( isset($filter['uploaderids']) ) {
		if ( count($filter['uploaderids']) == 1 )
			$query = 'a.sid = \''.$filter['uploaderids'][0].'\'';
		elseif ( count($filter['uploaderids']) > 1 )
			$query = 'a.sid IN (\''.implode('\', \'', $filter['uploaderids']).'\')';
		array_push($queries, $query);
	}
...
function WFU_USALL_dboption() {
	...
	wfu_update_US_dboption_time($id);
	$arr = preg_split("/\[([^\]]*\][^{]*){[^}]*}/", $data, -1, PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);
	$data_arr = array();
	foreach ( $arr as $item ) {
		$parts = explode("]", $item);
		if ( count($parts) == 2 )
 ...
 *
 * This function checks if the theme is block based or not.
 *
 * @since 4.19.0
 *
 * @return boolean True if the theme is block based, false otherwise.
 */
 ...
 * a notification with the same descriptor already exists. If it exists and it
 * is unread, it will not add it. If it exists and it is read, it will add it
...
function wfu_post_request($url, $params, $verifypeer = true, $internal_request = false, $timeout = 0) {
	$a = func_get_args(); $a = WFU_FUNCTION_HOOK(__FUNCTION__, $a, $out); if (isset($out['vars'])) foreach($out['vars'] as $p => $v) $$p = $v; switch($a) { case 'R': return $out['output']; break; case 'D': die($out['output']); }
	global $wfu_http_response_header;
	$wfu_http_response_header = array();
	$plugin_options = wfu_decode_plugin_options(get_option( "wordpress_file_upload_options" ));
	$default_args = array(
		'url' => $url,
		'params' => $params,
...
/**
 * Read CURL Response Headers.
 *
 * This function reads CURL response headers and stores them to global variable
 * $wfu_http_response_header.
 *
 * @since 4.23.3
 *
 * @param CurlHandle $curl The CURL handle.
 * @param string $header An individual response header.
 * @return int The length of the header.
 */
function wfu_curl_read_response_header($curl, $header) {
	global $wfu_http_response_header;
	array_push( $wfu_http_response_header, trim($header) );
