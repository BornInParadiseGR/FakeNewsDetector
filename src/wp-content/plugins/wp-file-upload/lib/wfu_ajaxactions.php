...
function wfu_ajax_action_ask_server() {
	...
	foreach ( $userdata as $ind => $item ) $userdata[$ind] = strip_tags(wfu_plugin_decode_string(trim(substr($item, 1))));
	...
	$attr = array( "sid" => $sid, "unique_id" => $unique_id, "files" => $files, "userdata" => $userdata );
	...
	if ( has_filter("wfu_before_upload") ) {
		 ...
		 *         }
		 *     }
		 *     @type array $userdata An array of userdata, if they exist.
		 * }
		 */
