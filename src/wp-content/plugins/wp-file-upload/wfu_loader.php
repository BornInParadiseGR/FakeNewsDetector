...
if ( !defined("ABSWPFILEUPLOAD_DIR") ) DEFINE("ABSWPFILEUPLOAD_DIR", plugin_dir_path( WPFILEUPLOAD_PLUGINFILE ));
...
add_action('wp_ajax_wfu_ajax_action_get_remotefiles_page', 'wfu_ajax_action_get_remotefiles_page');
...
function wordpress_file_upload_initialize() {
	...
	/**
	 * Execute Load Pre Actions.
	 *
	 * Allow extensions to execute custom actions before plugin initialization.
	 *
	 * @since 4.24.0
	 */
...
//			- d is the user name
	...
	if ( $js != "" ) {
		...
		$wfu_js_html = wfu_js_to_HTML($wfu_js);
		$btc = $params["blockcompatibility"];
		if ( $btc == "on" || ( $btc == "auto" && wfu_theme_is_block_based() ) ) {
			$instanceid = ( $post == null ? "" : $post->ID )."__".$sid;
	...
	//post process upload form inner HTML before sent to output
	/**
	 * Filter To Customise Uploader Inner Output.
	 *
	 * This filter is used to customise the inner HTML code generated by the
	 * plugin. The inner HTML is the HTML code of the plugin before it is
	 * processed by the templating system. It is useful when MaterialUI theme is
	 * active and it is required to send custom HTML code inside the shadow DOM.
	 *
	 * @since 4.23.0
	 *
	 * @param string $wordpress_file_upload_output_inner The HTML inner output.
	 * @param array $params An associative array with shortcode attributes.
	 */
	$wordpress_file_upload_output_inner = apply_filters("_wfu_file_upload_output_inner", $wordpress_file_upload_output_inner, $params);
...
/**
 * Generate the HTML code of Block Inline JS.
 *
 * It returns an HTML block containing Javascript code, which has been
 * previously stored in $WFU_BLOCK_INLINE_JS global variable. It is used when
 * the theme is block based. Block based themes do not render inline Javascript
 * code correctly, they escape ampersands. A workaround is to replace the inline
 * Javascript code with a shortcode, which outputs it at a later stage.
 *
 * @since 4.19.0
 *
 * @redeclarable
 ...
 *        attributes (array keys) and their values (array values).
 *
 * @return string The HTML block containing Javascript code.
 */
